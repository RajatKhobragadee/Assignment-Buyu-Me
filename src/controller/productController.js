    ...
    try{
      ...
      if(Array.isArray(data)){
       for(let i=0; i<data.length; i++){
        ...
        var saveData= await inventryModel.create(data)
    ...
    try{
        ...
        if(Array.isArray(data)){
            for(let i=0; i<data.length; i++){
              ...
              var alreadyExistData= await inventryModel.findOne({productId:productId})
            ...
            var updateData = await inventryModel.findOneAndUpdate({productId:productId},{$set:{quantity:response[i].quantity}},{new:true,upsert:true})
        ...
        var updateData = await inventryModel.findOneAndUpdate({productId:productId},{$set:{quantity:alreadyExistData.quantity}},{new:true,upsert:true})
        output.push(updateData)
     }
...
exports.getInventry =async (req, res)=>{
    const saveData= await inventryModel.find()
